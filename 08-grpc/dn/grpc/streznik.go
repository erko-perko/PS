...
type serverCRUD struct {
	...
	subscriber chan *protobufStorage.TodoEvent
}

// pripravimo nov strežnik CRUD za shrambo TodoStorage
func NewServerCRUD() *serverCRUD {
	todoStorePtr := storage.NewTodoStorage()
	subscriber := make(chan *protobufStorage.TodoEvent)
	return &serverCRUD{protobufStorage.UnimplementedCRUDServer{}, todoStorePtr, subscriber}
}
...
func (s *serverCRUD) Create(ctx context.Context, in *protobufStorage.Todo) (*emptypb.Empty, error) {
	...
	if s.subscriber != nil {
		s.subscriber <- &protobufStorage.TodoEvent{T: &protobufStorage.Todo{Task: in.Task, Completed: in.Completed}, Action: "create"}
	}
...
func (s *serverCRUD) Update(ctx context.Context, in *protobufStorage.Todo) (*emptypb.Empty, error) {
	...
	if s.subscriber != nil {
		s.subscriber <- &protobufStorage.TodoEvent{T: &protobufStorage.Todo{Task: in.Task, Completed: in.Completed}, Action: "update"}
	}
	return &emptypb.Empty{}, err
}
...
func (s *serverCRUD) Delete(ctx context.Context, in *protobufStorage.Todo) (*emptypb.Empty, error) {
	...
	if s.subscriber != nil {
		s.subscriber <- &protobufStorage.TodoEvent{T: &protobufStorage.Todo{Task: in.Task, Completed: in.Completed}, Action: "delete"}
	}
	return &emptypb.Empty{}, err
}

func (s *serverCRUD) Subscribe(_ *emptypb.Empty, stream protobufStorage.CRUD_SubscribeServer) error {
	eventChan := make(chan *protobufStorage.TodoEvent)
	s.subscriber = eventChan
	defer func() {
		s.subscriber = nil
		close(eventChan)
	}()
